...
mainpath_data = 'E:/Postgraduates/MetricLearning/TripletNetwork/OriginalData/Salinas'
mainpath_save = 'E:/Postgraduates/MetricLearning/TripletNetwork/TestResult/Salinas'
featurepath_save = 'E:/Postgraduates/MetricLearning/TripletNetwork/TestResult/Salinas/DMLOFTM/FeatureDimension/'
writer = pd.ExcelWriter(featurepath_save + 'Salinas_DMLOFM_85_Feature_Dimension_Margin1.xlsx')
for dim in range(1, 202, 10):
    if dim == 1:
        dim = dim
    else:
        dim = dim-1

    DataPath = mainpath_data + '/Salinas_corrected.mat'
    GtPath = mainpath_data + '/Salinas_gt.mat'
    TRPath = mainpath_data + '/Salinas_TRLabel_'+str(Number)+'.mat'
    TSPath = mainpath_data + '/Salinas_TSLabel_'+str(Number)+'.mat'

    tripletmodelpath = 'E:/Postgraduates/MetricLearning/TripletNetwork/TestResult/modelparameter/net_parameter_triplet_offline_'+str(dim)+'data_Salinas_margin1.pkl'
    classifiermodelpath = 'E:/Postgraduates/MetricLearning/TripletNetwork/TestResult/modelparameter/classifier_net_parameter_triplet_offline_'+str(dim)+'data_Salinas_margin1.pkl'
    ...
    for run in range(1, 11, 1):
        ...
        HSIdataset = torch.utils.data.TensorDataset(TrainData, TrainDataLabel)
        train_loader = torch.utils.data.DataLoader(HSIdataset, batch_size=batchsize, shuffle=True)
        triplet_train_dataset = TripletTrainData(HSIdataset)
        train_loader_triplet = torch.utils.data.DataLoader(triplet_train_dataset, batch_size=batchsize, shuffle=True)
        ...
        margin = 1
        embedding_net = EmbeddingNet(l, dim)
        ...
        fit(train_loader_triplet, model, loss_fn, optimizer, n_epochs, log_interval, BestAcc, dim)
        t1 = time.perf_counter()

        # train classifier network
        classifier = Classifier(l, dim, Classes).cuda()
        optimizer2 = optim.Adam(classifier.parameters(), lr = lr, weight_decay = 5*1e-4)
        ...
        classifier_fit(train_loader, classifier, loss_fn2, optimizer2, n_epochs2 ,log_interval, BestAcc2, dim)
...
writer.close()
    ...
    #     sio.savemat(mainpath_save + '/DMLOFTM/Dioni_DMLOFTM'+str(dim)+'/Dioni_'+str(dim)+'data_CNN_Triplet_offline'+str(run)+'.mat', {'PredAll': result, 'OA': OA * 100, 'TestPre': pred_y, 'TestLabel': TestDataLabel, 'Kappa':kappa})
    #     sp.save_rgb(mainpath_save + '/DMLOFTM/Dioni_DMLOFTM'+str(dim)+'/Dioni_'+str(dim)+'data_CNN_Triplet_offline'+str(run)+'.jpg', result, colors = dioni_colors)
    # f = open(mainpath_save + '/DMLOFTM/Dioni_DMLOFTM'+str(dim)+'/Dioni_'+str(dim)+'data_DMLOFTM__margin1_mean.txt','w')
    # print("OA均值：", np.mean(total_oa), "最大值：", np.max(total_oa), "最小值：", np.min(total_oa), "标准差：", np.std(total_oa), '\n',
    #         "Kappa均值：", np.mean(total_kappa), "最大值：", np.max(total_kappa), "最小值：", np.min(total_kappa),"标准差：", np.std(total_kappa), file = f) 
    # f.close()